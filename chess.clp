...
(deffunction read-pos (?str)
  ...
  (if (or (not (integerp ?num)) (< ?num 1) (> ?num 8))
      then (read-pos ?str)
      else ?num))
...
(deffunction enter-start-pos ()
  "Enter start position, if illegal (black king must no be under
attack) - reenter black king coordinates"
  (printout t "Starting a game. For exit - enter 'X' at any point." crlf)
  (if (not (and (bind ?wkv (read-pos "white king vertical position")) 
		(bind ?wkh (read-pos "white king horizontal position"))
		(bind ?wrv (read-pos "white rook vertical position"))
		(bind ?wrh (read-pos "white rook horizontal position"))))
      then (return FALSE))
  (while
      (and (= ?wkv ?wrv) (= ?wkh ?wrh))
    do
    (printout t "Illegal rook coordinates!" crlf)
    (if (not (and (bind ?wrv (read-pos "white rook vertical position"))
		  (bind ?wrh (read-pos "white rook horizontal position"))))
	then (return FALSE)))
  (if (not (and (bind ?bkv (read-pos "black king vertical position"))
		(bind ?bkh (read-pos "black king horizontal position"))))
      then (return FALSE))
  (while		;(or (stalemate ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
    ...
    (if (not (and (bind ?bkv (read-pos "black king vertical position"))
		  (bind ?bkh (read-pos "black king horizontal position"))))
	then (return FALSE)))
  (assert (figure (sortof king) (colour white) (vertical ?wkv) (horizontal ?wkh)))
  (assert (figure (sortof rook) (colour white) (vertical ?wrv) (horizontal ?wrh)))
  (assert (figure (sortof king) (colour black) (vertical ?bkv) (horizontal ?bkh)))
  TRUE)
...
(deffunction king-moves (?v ?h)
  ...
  (bind $?moves (create$))
  (if (regular-square (- ?v 1) (- ?h 1))
      then (bind ?moves (insert$ ?moves 1 (create$ (- ?v 1) (- ?h 1)))))
  (if (regular-square (- ?v 1) ?h)
      then (bind ?moves (insert$ ?moves 1 (create$ (- ?v 1) ?h))))
  (if (regular-square (- ?v 1) (+ ?h 1))
      then (bind ?moves (insert$ ?moves 1 (create$ (- ?v 1) (+ ?h 1)))))
  (if (regular-square ?v (- ?h 1))
      then (bind ?moves (insert$ ?moves 1 (create$ ?v (- ?h 1)))))
  (if (regular-square ?v (+ ?h 1))
      then (bind ?moves (insert$ ?moves 1 (create$ ?v (+ ?h 1)))))
  (if (regular-square (+ ?v 1) (- ?h 1))
      then (bind ?moves (insert$ ?moves 1 (create$ (+ ?v 1) (- ?h 1)))))
...
(depending on type of piece)."
  ...
  (loop-for-count (?rm-count 1 (div (length$ ?restrict-moves) 2))
     do
     (loop-for-count (?curr 1 (div (length$ ?moves) 2))
	do
	(if (and (= (nth$ (- (* 2 ?curr) 1) ?moves)
	    ...
	    then
	    (bind ?res-moves (replace$ ?res-moves
				       (- (* 2 ?curr) 1)
				       (* 2 ?curr) 0 0)))))
...
(deffunction king-defending-moves (?wrv ?wrh $?wk-moves)
  ...
  (loop-for-count (?curr 1 (div (length$ ?wk-moves) 2))
     do
     (if (or (> (abs (- (nth$ (- (* ?curr 2) 1) ?wk-moves) ?wrv))
		1)
	     (> (abs (- (nth$ (* ?curr 2) ?wk-moves) ?wrh))
		1))
	 then (bind ?wk-moves
		    (replace$ ?wk-moves (- (* ?curr 2) 1)
			      (* ?curr 2) 0 0))))
...
If rook is attacked - filter only defending moves if such."
  ...
  (if (and (< (abs (- ?bkh ?wkh)) 3) (< (abs (- ?bkv ?wkv)) 3))
      then (bind ?wk-moves (restrict-moves ?bkv ?bkh "king" ?wk-moves)))
  (if (and (< (abs (- ?wkh ?wrh)) 2) (< (abs (- ?wkv ?wrv)) 2))
      then (loop-for-count (?curr 1 (div (length$ ?wk-moves) 2)) do
	      (if (<= (* 2 ?curr) (length$ ?wk-moves))
		  then (if (and (= (nth$ (- (* 2 ?curr) 1) ?wk-moves) ?wrv)
				(= (nth$ (* 2 ?curr) ?wk-moves) ?wrh))
			   then (bind ?wk-moves (delete$ ?wk-moves (- (* 2 ?curr) 1) (* 2 ?curr)))
			   (break)))))
  (if (rook-attacked ?wrv ?wrh ?bkv ?bkh)
      then (bind ?wk-moves (king-defending-moves ?wrv ?wrh ?wk-moves)))
  ?wk-moves)

(deffunction restrict-rook (?wkv ?wkh ?wrv ?wrh $?moves)
  "Restrict possible moves by white rook in cases when white king is on the same
vertical or horizontal."
  (bind $?ret-moves (create$))
  (if (= ?wkv ?wrv)
      then (if (> ?wkh ?wrh)
	       then (loop-for-count (?curr 1 (div (length$ ?moves) 2)) do
		       (if (< (nth$ (* 2 ?curr) ?moves) ?wkh)
			   then (bind ?ret-moves (insert$ ?ret-moves 1
							  (create$ (nth$ (- (* 2 ?curr) 1) ?moves) (nth$ (* 2 ?curr) ?moves))))))
	       else (loop-for-count (?curr 1 (div (length$ ?moves) 2)) do
		       (if (> (nth$ (* 2 ?curr) ?moves) ?wkh)
			   then (bind ?ret-moves (insert$ ?ret-moves 1
							  (create$ (nth$ (- (* 2 ?curr) 1) ?moves) (nth$ (* 2 ?curr) ?moves)))))))
      else (if (> ?wkv ?wrv)
...
black king or defended by white king."
  ...
  (loop-for-count (?curr 1 (div (length$ ?moves) 2)) do
     (if (or (> (abs (- ?bkv (nth$ (- (* 2 ?curr) 1) ?moves))) 1)
	     (> (abs (- ?bkh (nth$ (* 2 ?curr) ?moves))) 1)
	     (and (< (abs (- ?wkv (nth$ (- (* 2 ?curr) 1) ?moves))) 2)
		  (< (abs (- ?wkh (nth$ (* 2 ?curr) ?moves))) 2)))
	 then (bind ?ret-moves (insert$ ?ret-moves 1
					(create$ (nth$ (- (* 2 ?curr) 1) ?moves) (nth$ (* 2 ?curr) ?moves))))))
  ?ret-moves)
...
(deffunction possible-rook-moves (?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
  ...
  (if (or (= ?wkv ?wrv) (= ?wkh ?wrh))
      then (bind ?wr-moves (restrict-rook ?wkv ?wkh ?wrv ?wrh ?wr-moves)))
  ?wr-moves)
...
(deffunction check-rook-moves (?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
  ...
  (if (or (< (abs (- ?bkh ?wrh)) 2) (< (abs (- ?bkv ?wkv)) 2))
      then (bind ?wr-moves (secure-rook-moves ?wkv ?wkh ?bkv ?bkh ?wr-moves)))
...
and white rook."
  ...
  (if (and (< (abs (- ?bkh ?wkh)) 3) (< (abs (- ?bkv ?wkv)) 3))
      then (bind ?bk-moves (restrict-moves ?wkv ?wkh "king" ?bk-moves)))
  (if (or (< (abs (- ?bkh ?wrh)) 2) (< (abs (- ?bkv ?wrv)) 2))
;      then (bind ?bk-moves (restrict-moves ?wrv ?wrh "rook"
      then
    (bind ?rook-moves (possible-rook-moves ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh))
    (bind ?i 1)
...
(deffunction play-only-black-move (?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
  ...
  (if (= (length$ ?moves) 2) then
    (assert (play-move (piece king) (colour black)
		       (vertical (nth$ 1 ?moves)) (horizontal (nth$ 2 ?moves))))
    (return TRUE))
  FALSE)
...
and has no possible moves."
  (and (or (= ?bkv ?wrv) (= ?bkh ?wrh))
       (= (length$ (check-black-moves ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)) 0)))
...
(deffunction find-region (?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
  "Find a continuous region arround given square (black king) of unattacked squares."
  (bind ?region (create$ ?bkv ?bkh (king-moves ?bkv ?bkh)))
  (bind ?i 2)
  (while (>= (length$ ?region) (* ?i 2)) do
	 (if (or (attacked-square ?wkv ?wkh ?wrv ?wrh (nth$ (- (* ?i 2) 1) ?region)
				  (nth$ (* ?i 2) ?region))
		 (and (controlled-square ?wkv ?wkh ?wrv ?wrh
					 (nth$ (- (* ?i 2) 1) ?region)
					 (nth$ (* ?i 2) ?region))
		      (> (get-distance ?bkv ?bkh (nth$ (- (* ?i 2) 1) ?region)
				       (nth$ (* ?i 2) ?region)) 1)))
	     then (bind ?region (delete$ ?region (- (* ?i 2) 1) (* ?i 2)))
	     else 
	     (bind ?new-moves (king-moves (nth$ (- (* ?i 2) 1) ?region)
					  (nth$ (* ?i 2) ?region)))
	     (bind ?j 1)
	     (while (>= (length$ ?new-moves) (* ?j 2)) do
		    (if (not (find-move-in-sequence (nth$ (- (* ?j 2) 1) ?new-moves)
						    (nth$ (* ?j 2) ?new-moves)
						    ?region))
			then (bind ?region (insert$ ?region (+ 1 (length$ ?region))
						    (nth$ (- (* ?j 2) 1) ?new-moves)
						    (nth$ (* ?j 2) ?new-moves))))
		    (bind ?j (+ ?j 1)))
	     (bind ?i (+ ?i 1))))
...
;							 ?bkv ?bkh)) 7)))
  ...
  (if (> ?diff 0) ;if rook is closer to black king...
      ...
      (bind ?estimate (+ ?estimate (* ?wr-bk-dist 4) (* ?diff 4)))
      else ;else - at least as close to white king, award nearness to kings
      (bind ?estimate (+ ?estimate (- ?wr-bk-dist 8) (- ?wr-wk-dist 8))))
  ?estimate)
...
Return best rook move only if it is better than the best king move."
  ...
  (if (< (estimation ?wkv ?wkh (nth$ 1 ?best-rook-move)
	 ...
	 (estimation (nth$ 1 ?best-move) (nth$ 2 ?best-move)
		     ?wrv ?wrh ?bkv ?bkh))
      then
      (assert (play-move (piece rook) (vertical (nth$ 1 ?best-rook-move))
...
(deffunction search-mate (?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
  ...
  (while (>= (length$ ?rook-moves) (* ?i 2)) do
	 (if (and (or (= ?bkv (nth$ (- (* ?i 2) 1) ?rook-moves))
		  ...
		  (check-for-mate ?wkv ?wkh (nth$ (- (* ?i 2) 1) ?rook-moves)
				  (nth$ (* ?i 2) ?rook-moves) ?bkv ?bkh))
	     then
	     (assert (play-move (piece rook)
				(vertical (nth$ (- (* ?i 2) 1) ?rook-moves))
				(horizontal (nth$ (* ?i 2) ?rook-moves))))
	     (return TRUE))
	 (bind ?i (+ ?i 1)))
  FALSE)
...
(deffunction print-move (?p ?v ?h ?delim)
  ...
  (printout t ?delim " " (piece-to-letter ?p) (vertical-to-letter ?v) ?h))
...
(deffunction enter-black-move (?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
  ...
  (while (not (find-move-in-sequence ?v ?h ?moves)) do
	 ...
	 (loop-for-count (?i 1 (div (length$ ?moves) 2))
	    (printout t (vertical-to-letter (nth$ (- (* ?i 2) 1) ?moves))
		      (nth$ (* ?i 2) ?moves) " "))
	 (printout t crlf)
	 (if (not (and
		   (bind ?v (read-pos "black king vertical"))
...
(deffunction make-move (?piece ?colour $?coord)
  ...
  (assert (play-move (piece ?piece) (colour ?colour)
		     (vertical (nth$ 1 ?coord)) (horizontal (nth$ 2 ?coord)))))
...
(defrule check-for-mate
  ...
  (figure (sortof king) (colour white) (vertical ?wkv) (horizontal ?wkh))
  (figure (sortof rook) (colour white) (vertical ?wrv) (horizontal ?wrh))
  (figure (sortof king) (colour black) (vertical ?bkv) (horizontal ?bkh))
  =>
...
- execute waiting move with white king."
  ...
  =>
  (make-move king white
	     (find-best-white-king-move ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh))
...
so play best rook move."
  ...
  =>
  (make-move rook white
	     (find-best-rook-move ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh))
  (modify ?s (value white-move-done)))
...
(defrule king-deffending-move
  ...
  (test (not (rook-must-move ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)))
  (test (not (on-edge ?wrv ?wrh)))
  (test (> (length$ (check-white-king-moves ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh))
	   0))
  =>
  (make-move king white
	     (find-best-white-king-move ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh))
